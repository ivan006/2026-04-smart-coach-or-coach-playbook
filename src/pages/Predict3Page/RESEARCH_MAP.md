...
These are the files that were read and contain the decision logic being translated into EQS.
...
| `field_analyzer.cpp`         | `/src/player/planner/field_analyzer.cpp`     | **Spatial utility methods used throughout all generators and scorers.** `can_shoot_from`: samples 10 angles across goal width, returns true if max angular gap from opponents ≥ 20° (self) / 15° (teammate), within 17 units of goal. `estimate_min_reach_cycle`: minimum cycle a player could intercept a ball trajectory. `predict_player_turn_cycle`: cycles needed to turn before dashing. Used in every safety check. |
